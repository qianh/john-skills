...
- **目标锚定（Goal Condition）**：将用户需求提炼为一条 `goal_condition` 字符串，格式：
  ...
  - 这是 N1 拷问的参照真北：所有拷问围绕「让这个条件成立需要做什么」展开。**N1 完成后，若拷问揭露的真实意图与初值不同，必须在进入 N3 前更新 goal_condition，并在 Gate 1 报告中展示最新值。**
  - 这是 N6 最终验收准则：N6 必须验证 goal_condition 成立。
  - 这是 N5 /goal 的条件来源（见 Step 4 N5）。
  - ⚠️ 条件推荐 **transcript-verifiable**：命令须在 N5 中被执行并打印，以便 /goal 评估器（Haiku）从 transcript 判断。**启用 /goal 时为硬约束；未启用时为推荐实践。** 不可写「实现符合设计原则」此类语义判断条件。
...
- **谁执行**：**不是编排器自己扫**。按 `registry.yaml` 的 `NS_scope_discovery.sub_steps.NS_B_scope` 绑定，**派子代理**（CC: `Explore` agent；Codex: `codebase-analyzer`）去发现：
  - 当前项目类型（前端 / 后端 / 服务 / 库）；
...
═══ /spec 编排报告（Gate 1）═══
🎯 需求：<一句话>
🏁 目标条件（goal_condition）：<transcript-verifiable 命令；N6 最终验收准则；空=未设定>
...
> **N5（实现）前必须**：AskUserQuestion 问 `subagent | 当前 agent`（铁律 7）；若 `goal_condition` 非空，同时询问是否启用 `/goal`（① 启用 ② 不启用）；再按所选模式委派 TDD 框架执行。
...
> 3. **goal_condition 最终验收**：运行 front-matter `goal_condition` 指定的命令（空字符串则跳过此项），输出与期望吻合则 N6 通过；否则 AskUserQuestion 提示「goal_condition 未满足，需重跑 N5」——**重跑 N5 后须重走 N7 审查（Gate 2 不可绕过）**，再回 N6。
