...
4. **Gate 1 是硬 STOP。** 必须用 AskUserQuestion 停下等用户确认，**绝不连跑过去**。Gate 2（N3 定稿 / N6 审查 / 合并前）同理。**Gate 审计记录（mandatory）：** 每次 Gate 通过后，必须立即在 `spec.md` 的「Gate 审计记录」表中写入该 Gate 行：时间戳（ISO）、决策摘要（用户选了什么/校准了什么）、确认方式。
...
- **漂移检测（续跑时）**：读 front-matter 的 `spec_commit` 字段（Gate 2 N3 定稿时写入的 commit hash，见 Gate 2 段落）。若字段非空，运行 `git log --oneline {spec_commit}..HEAD -- <spec 覆盖的文件路径列表>`；若有改动，用 AskUserQuestion 提示：「spec 锁定后代码已有 N 次变更，涉及 [文件列表]，spec 可能已漂移。① 先 review 再继续（推荐）② 直接续跑（忽略漂移）」。
...
0. **先打印声明行**（铁律 5）：`▶ {节点} | 框架: {name} | 模式: {Gate 1 选定的 mode，必须与 front-matter execution_modes 一致} | 状态: {bound|blocked}`。
...
4. **写节点执行追踪（mandatory）**：节点完成后，立即在 `spec.md`「节点执行追踪」表写入该行（列定义见 `spec-template.md`）。**调用证明**必填——如 `Skill("grill-with-docs")`、`Task("Explore")`、`native:命令`；为空 = 未被合法执行。
5. 更新 `spec.md` front-matter 的 `status`。
...
> 1. **cheapest-first 顺序 + 三态标记**：先单测（最快反馈）→ lint → typecheck → build → goal_condition 验收。**每项 DoD 只允许三种状态**：✅ 通过 / ❌ 失败 / ⏭️ BLOCKED(原因)。**不允许留空**——留空视为 N7 未完成。⏭️ BLOCKED 必须附原因（如「主应用依赖业务数据库连接，本地无法启动」）；BLOCKED 项是否豁免须在 N7 闸中 AskUserQuestion 让用户确认，用户豁免后追加 `waived:原因`，未豁免的 BLOCKED 项 = N7 未通过。
...
- **N3 规格定稿**（spec 是后续一切的判据）。**spec_commit 写入（mandatory）：** N3 定稿且用户确认后，立即执行 `git rev-parse --short HEAD` 并写入 `spec.md` front-matter 的 `spec_commit` 字段。此为后续漂移检测的基线——**spec_commit 为空 = N3 锁定痕迹不完整，status 不得进入 spec-locked**。
- **N6 审查**（防偏航/过度设计/安全）
...
### Step 6 · `done` 转换条件（mandatory gate，编排器最后一步）

`status` 从 `reviewing` 转为 `done` **必须同时满足以下全部条件**——缺任何一项则 `status` 停留在 `reviewing`，并在 front-matter 追加 `pending_items: [缺项列表]`：

1. **全 DoD 项已解决**：spec.md 验证记录（DoD）中每项为 ✅ 或 ⏭️BLOCKED+waived，不允许 ❌ 或留空。
2. **全 Gate 审计记录在位**：spec.md「Gate 审计记录」表中，本轮编排涉及的每个 Gate 行都已填写（不涉及的标 N/A）。
3. **全节点执行追踪已填**：spec.md「节点执行追踪」表中，每个编排节点都有调用证明记录。
4. **spec_commit 非空**（若本轮运行了 N3）：front-matter `spec_commit` 已写入 commit hash。
5. **goal_condition 已验收或已豁免**（若非空未豁免）。

不满足时，编排器必须输出缺项清单，用 AskUserQuestion 问用户：「① 补审计后再标 done（推荐）② 豁免缺项直接标 done（附理由）」。用户选②时在 front-matter 写入 `audit_waived: true` + `audit_waiver_reason: "..."` 再标 done。
